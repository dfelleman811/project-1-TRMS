...
create table employees (
    ...
    department varchar2(15) constraint d_names check (department in ('sales', 'research', 'design', 'marketing', 'relations')), -- relations = client relations
...
create table development_resources (
    ...
    start_date date, -- when the actual 'event' begins
    resource_time date, -- but what if it's a repeating event? i.e. a class that meets twice a week for a semester?
    resource_location varchar2(20), -- where the event is taking place
    resource_cost number(10), -- how much the 'event' costs
    grading_format varchar2(20) constraint g_type check (grading_format in ('pass/fail','graded', 'presentation')), 
...
foreign key (grading_format) references grading_references(format_id);
